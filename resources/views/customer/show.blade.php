...
@section('content')
                            ...
                            <ol class="breadcrumb" style="padding-left: 0px; font-weight: 400;">
                                ...
                                <li class="breadcrumb-item">
                                    @for ($i = 0; $i < count($productCategoryText); $i++)
                                        <a
                                            href="{{ url('customer/products?category=' . $productCategoryId[$i]) }}">{{ $productCategoryText[$i] }}</a>
    ...
    <section style="padding: 0;">
        <div class="container">
            <div class="row"
                style="background: #FFF; padding: 25px; margin: 0px; border: 1px solid rgba(0,0,0,.125); border-radius: .25rem;">
                            ...
                            <div class="col-md-12">
                                ...
                                <div class="radio-toolbar">
                                    @foreach ($product->units as $productUnit)
                                        <input type="radio" id="{{ $productUnit->unitName }}" id="unit"
                                            name="unit" value="{{ $productUnit->unitName }}">
                                        <label for="{{ $productUnit->unitName }}">
                                            {{ $productUnit->unitName }}
                                            @if (!$loop->first)
                                                : {{ $productUnit->quantityPerUnit }}
                                                {{ $product->units['0']['unitName'] }}
                                            @endif
                                            @if (count($product->promotions) != 0)
                                                @foreach ($product->promotions->reverse() as $promotion)
                                                    <div>ราคา
                                                        {{ number_format(doubleval($productUnit->pricePerUnit) - doubleval($promotion->name), 2) }}
                                                        บาท
                                                    </div>
                                                    @if ($promotion->name != '0')
                                                        <div style="text-decoration: line-through;">ราคา
                                                            {{ number_format($productUnit->pricePerUnit, 2) }}
                                                            บาท</div>
                                                    @endif
                                                    @if ($loop->iteration == 1)
                                                    @break
                ...
                @if ($product->quantity > 0)
                    <div class="buy mt-4">
                        <button class="btn btn-primary mr-3" id="addToCart"
                            @if (auth()->guard('admin')->check()) disabled @endif>
                            <i class="fa fa-shopping-cart"> <span
                                    style="font-weight: 300 !important;">เพิ่มใส่รถเข็น</span></i>
                        </button>
                    </div>
                @else
                    <div class="buy mt-4">
                ...
                <div class="col-sm-6 col-md-6 col-lg-4">
                    <div class="card card-product-grid">
                        ...
                        <figcaption class="info-wrap">
                            <div class="fix-height">
                                ...
                                @if (count($similarProduct->promotions) != 0)
                                    @foreach ($similarProduct->promotions->reverse() as $promotion)
                                        <div class="price mt-1">ราคา
                                            {{ number_format(doubleval($similarProduct->units['0']['pricePerUnit']) - doubleval($promotion->name), 2) }}
                                            บาท</div>
                                        @if ($promotion->name != '0')
                                            <div style="text-decoration: line-through;">ราคา
                                                {{ number_format($similarProduct->units['0']['pricePerUnit'], 2) }}
                                                บาท</div>
                                        @endif
                            ...
                            @else
                                <div class="price mt-1">ราคา
                                    {{ number_format($similarProduct->units['0']['pricePerUnit'], 2) }} บาท
                                </div>
                            @endif
                        </div>
                        <a id="{{ $similarProduct->slug }}"
                            @if ($similarProduct->quantity <= 0) href="#" class="btn btn-block btn-secondary mt-3" disabled
                            @else
                                href="{{ url('customer/products/' . $similarProduct->slug) }}" class="btn btn-block btn-primary mt-3" @endif
                            @if (auth()->guard('admin')->check()) disabled @endif>
    ...
    $("#buyProduct").click(function(e) {
        ...
        $.ajax({
            ...
            error: function(result) {
                ...
                $("#errorModal .modal-dialog .modal-content #message").html(result.responseJSON
                    .errors);
