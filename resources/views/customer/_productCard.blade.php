    ...
    <div class="card card-product-grid">
                ...
                @if (count($product->promotions) != 0)
                    @foreach ($product->promotions->reverse() as $promotion)
                        <div class="price mt-1">ราคา
                            {{ number_format(doubleval($product->units['0']['pricePerUnit']) - doubleval($promotion->name), 2) }}
                            บาท</div>
                        @if ($promotion->name != '0')
                            <div style="text-decoration: line-through;">ราคา
                                {{ number_format($product->units['0']['pricePerUnit'], 2) }} บาท</div>
                        @endif
        ...
        @else
            <div class="product-amount mt-3">
                <input class="product-quantity-{{ $product->slug }}" type="number" value="1" min="0"
                    max="{{ $product->quantity }}" step="1" />
            </div>
            <button class="btn btn-block btn-primary mt-3 addToCart" data-product_id="{{ $product->slug }}"
                data-product_name="{{ $product->name }}" data-product_unit="{{ $product->units['0']->unitName }}"
                @if (auth()->guard('admin')->check()) disabled @endif>
                เพิ่มใส่รถเข็น
            </button>
        @endif
        <a id="{{ $product->slug }}" href="{{ url('customer/products/' . $product->slug) }}"
            class="btn btn-block btn-secondary mt-3" @if (auth()->guard('admin')->check()) disabled @endif>
            ดูรายละเอียด
        </a>
    </figcaption>
</div>
            ...
            if (product_quantity <= 0) {
                ...
                $.ajax({
                    ...
                    error: function(result) {
                        ...
                        $("#errorModal .modal-dialog .modal-content #title").html(
                            "เกิดข้อผิดพลาด...");
                        $("#errorModal .modal-dialog .modal-content #message").html(result
                            .responseJSON.errors);
